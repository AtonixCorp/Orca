 ...
 *
 *     >>> import smartcito_fast
 *     >>> smartcito_fast.parse_frame(buf)
 *     {'version': 1, 'kind': 0, 'sensor_id': 42,
 *      'timestamp_ms': 1717000000000, 'value': 12.5}
 *
 * Build:   see native/README.md
 * License: Apache 2.0 (same as the rest of SmartCito)
 * ==========================================================================*/
 ...
 */
static PyObject *smartcito_parse_frame(PyObject *self, PyObject *args) {
    Py_buffer buf;
    (void)self;
...
static PyMethodDef SmartcitoMethods[] = {
    {"parse_frame", smartcito_parse_frame, METH_VARARGS,
     "Parse a 17-byte SmartCito binary sensor frame into a dict."},
    {NULL, NULL, 0, NULL}
};

static struct PyModuleDef smartcito_module = {
    PyModuleDef_HEAD_INIT,
    "smartcito_fast",
    "SmartCito C accelerators (binary frame parser).",
    -1,
    SmartcitoMethods,
    NULL, NULL, NULL, NULL
};

PyMODINIT_FUNC PyInit_smartcito_fast(void) {
    return PyModule_Create(&smartcito_module);
}
