...
var _ = Describe("Orca Controller", func() {
	Context("When reconciling a resource", func() {
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance Orca")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &OrcaReconciler{
				Client: k8sClient,
			...
			deploymentName := types.NamespacedName{Name: "test-resource-control-plane", Namespace: "default"}
