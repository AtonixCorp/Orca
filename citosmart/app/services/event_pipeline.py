...
class EventPipelineService:
    ...
    def enrich_event(self, event: NormalizedEvent) -> NormalizedEvent:
        ...
        metadata.setdefault(
            "geo_enriched",
            bool(event.payload.get("latitude") is not None and event.payload.get("longitude") is not None),
        )
        metadata.setdefault("time_bucket", event.occurred_at.strftime("%Y-%m-%dT%H:00:00Z"))
        metadata.setdefault("processed_by", "citosmart.event_pipeline")
    ...
    async def store_blob_if_present(self, event: NormalizedEvent) -> str | None:
        ...
        if not isinstance(blob_text, str) or not blob_text:
            return None
        return await object_storage_service.store_blob(data=blob_text.encode("utf-8"), suffix=".txt")
    ...
    async def emit_pipeline_events(
        ...
        *,
        ...
        if publisher is not None:
            await publisher.publish_event(clean_event.model_dump(mode="json"), topic=self._topics["clean_events"])
        ...
        if publisher is not None:
            await publisher.publish_event(alert.model_dump(mode="json"), topic=self._topics["alerts"])
        return alert
    ...
    async def process_sensor_reading(
        ...
        *,
        ...
        if publisher is not None:
            await publisher.publish_event(raw_event.model_dump(mode="json"), topic=self._topics["raw_events"])
